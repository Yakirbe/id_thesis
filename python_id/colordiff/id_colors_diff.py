...
def get_start_stop(len_ds = 13500, length = 1000):
        start = 0
        stop = length
        ...
        del start_stop[-1]
        if start_stop[-1][1] < len_ds:
            start = start_stop[-1][1] + 1
            stop = len_ds
            start_stop.append([start,stop])
...
if __name__ == "__main__":
    ...
    ds_c_vec = random.sample(ds,5000)
    ...
    cvecs = gen_C_set(ds_c_vec , dis_min , dis_max , vec_len = len(ds[0]["vec"]) , min_max_margin = 0.01)    
        
    start_stop = get_start_stop(len_ds = len(ds))
